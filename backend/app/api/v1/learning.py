...
from app.models import FavoriteWord, LanguageProfile, LearningCard, ReviewLog, SrsState, User, UserSettings
...
async def create_card(
    ...
    existing = await db.execute(
        select(LearningCard).where(
            ...
            LearningCard.lemma_l2 == body.lemma,
            LearningCard.deck_id.is_(None),
        )
    )
    if existing.scalar_one_or_none() is not None:
        raise HTTPException(status_code=409, detail="To słowo jest już w Twojej nauce")
...
async def srs_queue(
    ...
    due_result = await db.execute(
        ...
        .where(
            ...
            SrsState.scope == "main",
            SrsState.status.in_(["learning", "review"]),
